...
namespace CallOfTheWild.HoldingItemsMechanics
{
    ...
    class ItemEntityWeapon__HoldInTwoHands__Patch
    {
        static void Postfix(ItemEntityWeapon __instance, ref bool __result)
        {
            ...
            if (
                __instance.Blueprint.IsTwoHanded
            ...
            )
            {
                ...
                else
                {
                    ...
                    var use_spell_combat_part =
                        __instance.Wielder?.Get<UnitPartCanUseSpellCombat>();
                    ...
                    __result = use_spell_combat_part.canBeUsedOn(
                        __instance.HoldingSlot as HandSlot,
                        pair_slot,
                        true
                    );
                }
        ...
        static bool Prefix(HandSlot __instance)
        {
            ...
            if (!__instance.IsPrimaryHand)
            {
                ...
                if (
                    maybeItem != null
                    && (
                        (maybeItem.Blueprint.IsTwoHanded && !unit_part.canBeUsedOn(maybeItem))
                        || maybeItem.Blueprint.Double
                    )
                )
                {
            ...
            if (
                maybeItem1 != null
                && (
                    (maybeItem1.Blueprint.IsTwoHanded && !unit_part.canBeUsedOn(maybeItem1))
                    || maybeItem1.Blueprint.Double
                )
            )
            {
                if (__instance.IsPrimaryHand)
    ...
    public class UnitPartCanHold2hWeaponIn1h : AdditiveUnitPart
    {
        ...
        public bool canBeUsedAs2h(ItemEntityWeapon weapon)
        {
            ...
            foreach (var b in buffs)
            {
                bool can_use = false;
                bool can_2h = false;
                b.CallComponents<CanUse2hWeaponAs1hBase>(c =>
                {
                    can_2h = c.canBeUsedAs2h(weapon);
                    can_use = c.canBeUsedOn(weapon);
                });
                if (can_use && can_2h)
                {
                    return true;
    ...
    {
        public abstract bool canBeUsedOn(ItemEntityWeapon weapon);

        public abstract bool canBeUsedAs2h(ItemEntityWeapon weapon);
    ...
    public class UnarmedTwf : OwnedGameLogicComponent<UnitDescriptor>
    {
        public override void OnFactActivate()
        {
            ...
            EventBus.RaiseEvent<IUnitActiveEquipmentSetHandler>(
                (Action<IUnitActiveEquipmentSetHandler>)(
                    h => h.HandleUnitChangeActiveEquipmentSet(this.Owner)
                )
            );
        }
    ...
    public class UnitPartCanUseSpellCombat : AdditiveUnitPart
    {
        public bool canBeUsedOn(HandSlot primary_hand, HandSlot secondary_hand, bool use_two_handed)
        {
            ...
            {
                bool result = false;
                b.CallComponents<CanUseSpellCombatBase>(c =>
                    result = c.canBeUsedOn(primary_hand, secondary_hand, use_two_handed)
                );
                if (result)
                {
                    return true;
    ...
    {
        public abstract bool canBeUsedOn(
            HandSlot primary_hand_slot,
            HandSlot secondary_hand_slot,
            bool use_two_handed
        );
    ...
    {
        public override bool canBeUsedOn(
            HandSlot primary_hand_slot,
            HandSlot secondary_hand_slot,
            bool use_two_handed
        )
    ...
    public class UseSpellCombatWith2ManifestedWeapons : CanUseSpellCombatBase
    {
        ...
        )
        {
            ...
            return weapon.EnchantmentsCollection.HasFact(required_enchant)
                && weapon2.EnchantmentsCollection.HasFact(required_enchant);
        }
            ...
            IInitiatorRulebookHandler<RuleCalculateAttackBonusWithoutTarget>
    {
        ...
        public override bool canBeUsedOn(ItemEntityWeapon weapon)
        {
            ...
            var shield_proficiency = holding_slot
                .PairSlot
                .MaybeShield
                .ArmorComponent
                .Blueprint
            ...
            if (
                shield_proficiency
                == Kingmaker.Blueprints.Items.Armors.ArmorProficiencyGroup.Buckler
            )
            {
                return false;
            }

            return weapon.Blueprint.FighterGroup == WeaponFighterGroup.Spears
                || weapon.Blueprint.FighterGroup == WeaponFighterGroup.Polearms;
        }
        ...
        public void OnEventAboutToTrigger(RuleCalculateAttackBonusWithoutTarget evt)
        {
            ...
            int penalty = Rulebook
                .Trigger<RuleCalculateArmorCheckPenalty>(
                    new RuleCalculateArmorCheckPenalty(evt.Initiator, shield.ArmorComponent)
                )
                .Penalty;
    ...
    public class UnhinderingShield : CanUse2hWeaponAs1hBase
    {
        ...
        public override bool canBeUsedOn(ItemEntityWeapon weapon)
        {
            ...
            return holding_slot.PairSlot.MaybeShield.ArmorComponent.Blueprint.ProficiencyGroup
                == Kingmaker.Blueprints.Items.Armors.ArmorProficiencyGroup.Buckler;
        }
    }
    ...
    public class CanHoldIn1Hand : CanUse2hWeaponAs1hBase
    {
        ...
        public override bool canBeUsedOn(ItemEntityWeapon weapon)
        {
            if (
                ...
                && !(
                    weapon
                        .Owner?.Get<WeaponsFix.UnitPartFullProficiency>()
    ...
    internal class Helpers
    {
        internal static bool hasFreeHand(HandSlot hand_slot)
        {
            ...
            return shield.ArmorComponent.Blueprint.ProficiencyGroup
                    == Kingmaker.Blueprints.Items.Armors.ArmorProficiencyGroup.Buckler
                && hand_slot.Owner.HasFact(NewFeats.unhindering_shield);
        }
    ...
    class UnitPartMagus__HasOneHandedMeleeWeaponAndFreehand__Patch
    {
        static void Postfix(UnitDescriptor unit, ref bool __result)
        {
            ...
            {
                __result =
                    !unit.Body.PrimaryHand.MaybeWeapon.HoldInTwoHands
                    && Helpers.hasFreeHand(unit.Body.SecondaryHand);
            }
    ...
    class MonkNoArmorAndMonkWeaponFeatureUnlock__CheckEligibility__Patch
    {
        ...
        )
        {
            ...
            var check_shield = codes.FindIndex(x =>
                x.opcode == System.Reflection.Emit.OpCodes.Callvirt
                && x.operand.ToString().Contains("HasShield")
            );

            codes[check_shield] = new Harmony12.CodeInstruction(
                System.Reflection.Emit.OpCodes.Call,
                new Func<HandSlot, bool>(Helpers.hasShield2).Method
            );
    ...
    class MonkNoArmorFeatureUnlock__CheckEligibility__Patch
    {
        static bool Prefix(MonkNoArmorFeatureUnlock __instance)
        {
            Main.TraceLog();
            if (
                !Helpers.hasShield2(__instance.Owner.Body.SecondaryHand)
                && (
                    !__instance.Owner.Body.Armor.HasArmor
                    || !__instance.Owner.Body.Armor.Armor.Blueprint.IsArmor
                )
            )
            {
                __instance.AddFact();
    ...
    class DeflectArrows__CheckRestriction__Patch
    {
        ...
        )
        {
            ...
            check_shield = codes.FindIndex(x =>
                x.opcode == System.Reflection.Emit.OpCodes.Callvirt
                && x.operand.ToString().Contains("HasShield")
            );
            codes[check_shield] = new Harmony12.CodeInstruction(
    ...
    class RestrictionCanGatherPower__IsAvailable__Patch
    {
        ...
        static bool getResult(RestrictionCanGatherPower __instance)
        {
            ...
            bool flag =
                category.GetValueOrDefault() == WeaponCategory.KineticBlast && category.HasValue;
            ...
            if (maybeItem2 == null)
                return true;
            ArmorProficiencyGroup? proficiencyGroup = body.SecondaryHand
                .MaybeShield
                ?.Blueprint
                .Type
                .ProficiencyGroup;
            ...
            if (
                proficiencyGroup.HasValue
                && (proficiencyGroup.GetValueOrDefault() != ArmorProficiencyGroup.TowerShield)
            )
                return unitPartKineticist.CanGatherPowerWithShield
                    || Helpers.hasFreeHand(body.SecondaryHand);
            return false;
        }
    }
    ...
    {
        private static BlueprintParametrizedFeature slashing_grace =
            Main.library.Get<BlueprintParametrizedFeature>("697d64669eb2c0543abb9c9b07998a38");

        private static BlueprintParametrizedFeature fencing_grace =
            Main.library.Get<BlueprintParametrizedFeature>("47b352ea0f73c354aba777945760b441");

        private static BlueprintFeature deft_grace = Main.library.Get<BlueprintFeature>(
            "b63a316cb172c7b4e906a318a0621c2c"
        );

        private static BlueprintFeature stalker_grace = Main.library.Get<BlueprintFeature>(
        ...
        static void Postfix(
            ...
            bool forDollRoom,
            ref WeaponAnimationStyle __result
        )
        {
            Main.TraceLog();
            if (__instance == null)
            ...
            {
                HandSlot handSlot = (__instance.HoldingSlot as HandSlot)?.PairSlot;
                if (
                    __instance.Blueprint.IsOneHandedWhichCanBeUsedWithTwoHands
                    && handSlot?.HasItem != true
                )
                {
                    if (
                        __result == WeaponAnimationStyle.SlashingTwoHanded
                        && (
                            __instance.Owner.Progression.Features.HasFact(slashing_grace)
                            || __instance.Owner.Progression.Features.HasFact(stalker_grace)
                        )
                    )
                    {
                        ...
                        && (
                            __instance.Owner.Progression.Features.HasFact(fencing_grace)
                            || __instance.Owner.Progression.Features.HasFact(stalker_grace)
                        )
                    )
                    {
                        ...
                        __instance.Blueprint.Category == WeaponCategory.DuelingSword
                        && __instance.Owner.Progression.Features.HasFact(deft_grace)
                    )
                    {
                        __result = WeaponAnimationStyle.SlashingOneHanded;
                    }
                ...
                else if (__instance.Blueprint.Category == WeaponCategory.Shortsword)
                {
                    __result = WeaponAnimationStyle.Dagger;
                }
            }
            else if (
                (
                    (__instance.Blueprint.IsTwoHanded && !__instance.HoldInTwoHands)
                    || (
                        (
                            __instance.Blueprint.IsTwoHanded
                            || __instance.Blueprint.IsOneHandedWhichCanBeUsedWithTwoHands
                ...
                && ((__instance.HoldingSlot as HandSlot)?.PairSlot)?.HasItem == true
            )
            {
                switch (__result)
                {
    ...
    public class UnitBody__SetEmptyHandWeapon__Patch
    {
        ...
        static void Postfix(UnitBody __instance, BlueprintItemWeapon weapon)
        {
            ...
            EventBus.RaiseEvent<IUnitActiveEquipmentSetHandler>(
                (Action<IUnitActiveEquipmentSetHandler>)(
                    h => h.HandleUnitChangeActiveEquipmentSet(__instance.Owner)
                )
            );
    ...
    class AddMechanicsFeature__OnTurnOn__Patch
    {
        ...
        {
            var feature = CallOfTheWild.Helpers.GetField<AddMechanicsFeature.MechanicsFeatureType>(
                __instance,
                "m_Feature"
            );
    ...
    class WeaponSlot_CanRemoveItem_Patch
    {
        static void Postfix(WeaponSlot __instance, ref bool __result)
        {
            ...
            if (!__result)
            {
                ...
                {
                    if (
                        NewMechanics.EnchantmentMechanics.Helpers.isSummoned(__instance.MaybeItem)
    ...
    class CharSAttack__SetupMainAttacks__Patch
    {
        ...
        {
            var codes = instructions.ToList();
            var check_is_unarmed = codes.FindIndex(x =>
                x.opcode == System.Reflection.Emit.OpCodes.Callvirt
                && x.operand.ToString().Contains("IsUnarmed")
            ); //checking is unarmed on weapon on primary hand

            codes[check_is_unarmed] = new Harmony12.CodeInstruction(
            ...
            codes.Insert(
                ...
                new Harmony12.CodeInstruction(
                    System.Reflection.Emit.OpCodes.Call,
                    new Func<BlueprintItemWeapon, UnitDescriptor, bool>(
                        considerUnarmedAndIgnore
                    ).Method
                )
            );

            check_is_unarmed = codes.FindIndex(x =>
                x.opcode == System.Reflection.Emit.OpCodes.Callvirt
                && x.operand.ToString().Contains("IsUnarmed")
            ); //checking is unarmed on weapon on secondary hand
            codes[check_is_unarmed] = new Harmony12.CodeInstruction(
                System.Reflection.Emit.OpCodes.Ldarg_1
            );
            codes.Insert(
                ...
                new Harmony12.CodeInstruction(
                    System.Reflection.Emit.OpCodes.Call,
                    new Func<BlueprintItemWeapon, UnitDescriptor, bool>(
                        considerUnarmedAndIgnoreOffHand
                    ).Method
                )
            );
            return codes.AsEnumerable();
        ...
        private static bool considerUnarmedAndIgnore(
        ...
        private static bool considerUnarmedAndIgnoreOffHand(
            BlueprintItemWeapon weapon,
            UnitDescriptor unit
        )
        {
    ...
    class RuleCalculateAttacksCount__OnTrigger__Patch
    {
        ...
        static bool Prefix(RuleCalculateAttacksCount __instance, RulebookEventContext context)
        {
            ...
            BlueprintItemWeapon blueprint2 =
                secondary_hand.MaybeShield != null
                    ? (
                        (bool)__instance.Initiator.Descriptor.State.Features.ShieldBash
                            ? secondary_hand.MaybeShield.WeaponComponent?.Blueprint
                            : null
                    )
                    : secondary_hand.MaybeWeapon?.Blueprint;
            int num =
                primary_hand.MaybeWeapon == null
                    ? 0
                    : (primary_hand.MaybeWeapon.HoldInTwoHands ? 1 : 0);
            if (
                (secondary_hand.MaybeWeapon?.HoldInTwoHands).GetValueOrDefault() == false
                && (blueprint1 != null)
                && (
                    !considerUnarmedAndIgnore(blueprint1, __instance)
                    || blueprint2 == null
            ...
            )
            {
                ...
                if (
                    !blueprint1.IsNatural
                    || (bool)__instance.Initiator.Descriptor.State.Features.IterativeNaturalAttacks
                    || __instance.ForceIterativeNaturealAttacks
                    || blueprint1.IsUnarmed
                )
                    __instance.PrimaryHand.PenalizedAttacks += Math.Max(0, num_penalized_attacks);
            }
            if (
                (secondary_hand.MaybeWeapon?.HoldInTwoHands).GetValueOrDefault() == true
                || (blueprint2 == null)
            ...
            if (
                ...
                || !considerUnarmedAndIgnore(blueprint1, __instance)
                    && (
                        ...
                        || !blueprint1.IsNatural
                        || !(bool)
                            __instance.Initiator.Descriptor.State.Features.IterativeNaturalAttacks
                            && !__instance.ForceIterativeNaturealAttacks
                    )
                || (blueprint2.IsUnarmed)
            )
    ...
    class UnitAttack__TriggerAttackRule__Patch
    {
        ...
        {
            TwoWeaponFightingDamagePenalty__OnEventAboutToTrigger__Patch.is_off_hand = (
                attack?.Hand != null && attack?.Hand == __instance.Executor?.Body?.SecondaryHand
            );
            return true;
        }
            ...
            if (
                ...
                || (
                    ...
                    && (
                        ...
                        || Aux.isMainHandUnarmedAndCanBeIgnored(
                ...
                || (
                    ...
                    && (
                        ...
                        || Aux.isOffHandUnarmedAndCanBeIgnored(
    ...
    {
        public static bool isMainHandUnarmedAndCanBeIgnored(
            BlueprintItemWeapon weapon,
            UnitDescriptor unit
        )
        {
        ...
        public static bool isOffHandUnarmedAndCanBeIgnored(
            BlueprintItemWeapon weapon,
            UnitDescriptor unit
        )
        {
            return weapon.IsUnarmed
                && !(
                    (bool)unit.State.Features.ImprovedUnarmedStrike
                    && (unit.Get<UnitPartUnarmedTwf>()?.active()).GetValueOrDefault()
                );
        }
    }
}
